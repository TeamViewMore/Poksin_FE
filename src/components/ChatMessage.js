...
const ChatMessage = ({ message, loggedInUser }) => {
    ...
    const renderFileMessage = (fileUrl, fileName) => {
        ...
        if (imageExtensions.includes(fileExtension)) {
            ...
            return <video controls><source src={fileUrl} type={`video/${fileExtension}`} />Your browser does not support the video tag.</video>;
        } else if (audioExtensions.includes(fileExtension)) {
            return <audio controls><source src={fileUrl} type={`audio/${fileExtension}`} />Your browser does not support the audio tag.</audio>;
        } else {
        ...
        <div style={{marginBottom: '11px',}}>
            ...
            >
                <C.Message style={{
                    ...
                    {type === 'FILE' ? (
                        renderFileMessage(fileUrl, fileName)
                    ) : (
                        <div className='text'>{msgText}</div>
                    )}
                </C.Message>
            </C.All>
            <C.Text style={{justifyContent: isSender ? 'flex-end' : 'flex-start',}}>{formatTimestamp(timestamp)}</C.Text>
        </div>
    );
};
